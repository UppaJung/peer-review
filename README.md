...
From the perspective of those in criminal justice – with all its flaws that corrupt and bias bias it be decidedly unjust – how much more corrupt science must seem; first, for allowing researchers to both investigate a hypothesis and advocate a conclusion; and second, for placing final judgement in the hands of anonymous investigators, some of whom may be advocated (or have advocated) similar or competing conclusions.

If we learned from the criminal justice system, we would not encourage scientists to draw judgments beyond that which can be inferred by the reader from the evidence presented. We would also question the wisdom of condensing peers' insights into a boolean judgments of the validity of a work, and treating one of those outcomes as scientific consensus that the work is credible.
...
<!-- SD: Given that PhD admissions and RPT incentive structures will be slow to update, as long as authors are incentivized to inflate publication "counts" leaving the "publication" decision solely up to authors may result in adversarial behavior? Perhaps the new role of organizations like ACM should be to maintain the infrastructure for elucidative peer review with accompanying annotations (e.g., whether it *has* been reviewed)? -->

Using elucidative peer review, we could even submit our experimental methodology for feedback, using reviewers' insights to improve those experimental designs *prior* to running experiments and reporting results. <!-- SD: would this be an update of the current suggested practice of pre-registration? Elucidative reviews on pre-registered experiments? --> We could then seek further feedback after completing the research. Currently, peer review only reveals experimental design flaws after we have spent all the resources needed to run an experiment, and often after key co-authors have graduated or started other jobs.
